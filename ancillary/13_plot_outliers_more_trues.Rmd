...
this_truth_definition <- "outlier_present_in_shifted_literature" #outlier_defines_disease_by_ttest" # "outlier_present_in_literature" # "outlier_present_in_shuffled_literature" "outlier_not_present_in_literature" "95_random_outliers_are_true"
...
annotated_outlier_results <- lapply(unique(outlier_results_raw$parent_sample), function(this_parent_sample) {
  ...
  if (this_truth_definition == "found_in_the_deepest_subset") {
	...
	if (this_truth_definition == "outlier_present_in_shuffled_literature") {
	  # number of outliers found for this sample
	  set.seed(1)
	  
	  shuffled_literature_outliers <- literature_outliers %>%
	    mutate(Eval_ID = sample(Eval_ID, length(Eval_ID)))
	  
	  true_outliers_in_parent_sample <- left_join(outlier_results, shuffled_literature_outliers, 
	                                              by=c("parent_sample"="Eval_ID", "gene")) %>%
	    filter(n_hits_per_gene>0) %>%
	    pull(gene) %>% unique
	}
	
		
	if (this_truth_definition == "outlier_present_in_literature") {
	  # number of outliers found for this sample
	  true_outliers_in_parent_sample <- left_join(outlier_results, literature_outliers, 
	                                              by=c("parent_sample"="Eval_ID", "gene")) %>%
	    filter(n_hits_per_gene>0) %>%
	    pull(gene) %>% unique
	}
		
	if (this_truth_definition == "outlier_not_present_in_literature") {
	  # number of outliers found for this sample
	  true_outliers_in_parent_sample <- left_join(outlier_results, literature_outliers, 
	                                              by=c("parent_sample"="Eval_ID", "gene")) %>%
	    filter(is.na(n_hits_per_gene)) %>%
	    pull(gene) %>% unique
	}
	
	  
  if (this_truth_definition == "outlier_present_in_shifted_literature") {
    # number of outliers found for this sample
    other_samples_literature_outliers <- subset(literature_outliers, Eval_ID != this_parent_sample )
    candidate_true_outliers_in_parent_sample <- outlier_results %>%
      filter(gene %in% other_samples_literature_outliers$gene) %>%
      select(gene) %>% 
      distinct
    
    true_outliers_in_parent_sample <- candidate_true_outliers_in_parent_sample %>%
      filter(!gene %in% subset(literature_outliers, Eval_ID == this_parent_sample )$gene) %>%
      pull(gene)
  }
  
		
	if (this_truth_definition == "95_random_outliers_are_true") {
	  # number of outliers found for this sample
	  set.seed(this_random_seed)
	  true_outliers_in_parent_sample <- sample(unique(outlier_results$gene),95/5)
  ...
  if (this_truth_definition != "found_in_the_deepest_subset") {
	  outlier_results <- outlier_results %>% 
	    ...
	    dplyr::filter(gene %in% true_outliers_in_parent_sample)
  }
	 ...
	 if (this_truth_definition %in% c("outlier_present_in_literature", "outlier_defines_disease_by_ttest")) {
	   this_annotated_outlier_results <- this_annotated_outlier_results %>%
	     filter(gene %in% true_outliers_in_parent_sample)
	 }
...
for (this_parent_sample in unique(ordered_plot_input$parent_sample)) {
  ...
  cat(count_of_positives_in_full_set, file = paste0("count_of_true_outliers_", this_parent_sample, "_", this_truth_definition))
...
this_truth_definition
plot_title=paste("outlier status of", length(unique(ordered_plot_input$ordered_gene)), "genes across",length(unique(ordered_plot_input$parent_sample)), "samples - the random seed is", this_random_seed)
#plot_subtitle = paste(this_truth_definition "outlier status of", length(unique(ordered_plot_input$ordered_gene)), "genes across",length(unique(ordered_plot_input$parent_sample)), "samples")
pIA<-ggplot(ordered_plot_input, aes(x = median_of_depth_bin, y = ordered_gene, fill = hit_type)) + 
...
# Identify genes that don't have good outlier support
```{r}
outlier_support_summary <- ordered_plot_input %>% 
  group_by(parent_sample, gene) %>%
  summarize(n_outlier_support = sum(is_outlier_in_subset)) %>%
  arrange(n_outlier_support)


table(outlier_support_summary$n_outlier_support)

write_tsv(outlier_support_summary, paste0("outlier_support_summary_", this_truth_definition, "_rs_", this_random_seed, ".tsv"))
...
# Table of accuracy stats - per parent samples
...
accuracy_stats_sens_spec %>%
  select(parent_sample, median_of_depth_bin, measurement, value) %>%
  spread(measurement, value) %>% 
  select(-specificity) %>%
write_tsv(paste0("outlier_median_accuracy_stats_", this_truth_definition, "_rs_", this_random_seed, ".tsv"))
