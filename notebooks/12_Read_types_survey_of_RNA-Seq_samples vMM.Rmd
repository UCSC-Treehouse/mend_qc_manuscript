...
read_type_training_set <- plotInfo %>% 
  group_by(sample_id) %>%
  dplyr::filter(read_countM[read_type == "UMEND"] > 20)

# View(read_type_training_set)

rtMM <- read_type_training_set %>% filter(read_type=="Multi-mapped") %>%
  pull(percent_of_total)
mean(rtMM)
summary(multimapped_of_mapped$pct_multimapped_of_mapped)


nrow(read_type_training_set)
nrow(multimapped_of_mapped)
multimapped_of_mapped$sample_id[! multimapped_of_mapped$sample_id %in% read_type_training_set$sample_id]


subset(multimapped_of_mapped, sample_id =="TARGET-40-0A4HLD-01A-01R")
...
# Define reference range for mulitmapped reads
```{r}

multimapped_of_mapped <- complete_counts_raw %>%
  dplyr::filter(UMEND > 20*1E6) %>% 
...
```{r}
combined_thresholds <- read_type_pct_thresholds %>% bind_rows(tibble(read_type="Multimappers", min_pct =multimapper_thresholds$min_pct, max_pct =multimapper_thresholds$max_pct ))
```



# plot multimapped distribiution and limits
```{r}
...
plot_info_with_limits <- left_join(plotInfo, combined_thresholds, by = "read_type") %>%
...
length(samples_within_read_type_limits[!samples_within_read_type_limits %in% samples_within_multimapper_limits])

# samples within multimapper limits but not read type limits
length(samples_within_multimapper_limits[!samples_within_multimapper_limits %in% samples_within_read_type_limits])
...
plot_info_for_problem_samples_min <- plot_info_for_problem_samples %>% select(read_type, sample_id, within_limits, status)

# add MM
plot_info_for_problem_samples_with_mm <- plot_info_with_limits2 %>% 
  select(sample_id, status) %>% 
  distinct %>%
  mutate(read_type = "multi-mapper subtype",
         within_limits = sample_id %in% samples_within_multimapper_limits)
...
threshold_codes=c("NM"="Not mapped", "D"="Duplicate reads", "NE" = "Non exonic reads", "U" = "UMEND")
