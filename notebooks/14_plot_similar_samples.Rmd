...
first_degree_results_with_corrs_gt_875 <- read_tsv("../data/correlated_samples.txt",
                                                   col_types = cols()) %>% 
  rename(neighbor_sample = first_degree_neighbor_sample) %>%
  mutate(
    sample_is_neighbor_in_subset = TRUE, 
    parent_sample = gsub("_est.*$", "", focus_sample),
    target_MEND_count = as.numeric(gsub("^.*_est([0-9]*)M_.*$", "\\1", focus_sample))) %>%
...
disease_info_for_compendium <- read_tsv("../data/TreehousePEDv9_clinical_metadata.2019-03-15.tsv", col_types=cols()) %>%
  rename(THid = th_sampleid)
...
plot_colors <- c("False positive" = "red",
                 "True negative" = "pink",
...
plot_colors <- c("False positive" = cb_light_orange,
                 "True negative" = cb_dark_orange,
                 "True positive" = cb_dark_purple,
                 "False negative" = cb_light_purple )
...
first_degree_results <- first_degree_results_plus %>%
  ...
  left_join(disease_info_for_compendium %>%
              select (THid, disease),
...
annotated_fdn_results_raw <- lapply(unique(first_degree_results_filtered$parent_sample), 
  ...
  # this_parent_sample ="TH_Eval_019"
	...
	generic_subsample_info <- fdn_found_in_selected_subsamples %>% 
	  ...
	  evenly_selected_subsamples_this_parent <- subset(evenly_selected_subsamples,
	                                                   parent_id == this_parent_sample) 
	
	empty_results <- crossing(evenly_selected_subsamples_this_parent,
	                          neighbor_sample = samples_that_are_fdn_in_any_subset)
...
#	  mutate(neighbor_sample = rep(samples_that_are_fdn_in_any_subset, each = nrow(evenly_selected_subsamples_this_parent)))
  ...
  fdn_results <- right_join(fdn_found_in_selected_subsamples %>%
	                            select(-target_MEND_count,
	                                   -median_of_depth_bin,
	                                   -publication_id),
                            empty_results,
	                          by = c("parent_sample" = "parent_id",
	                                 "focus_sample" = "sample_id",
	                                 "neighbor_sample")) %>%
    mutate(sample_is_neighbor_in_subset = ifelse(is.na(sample_is_neighbor_in_subset),
                                                 FALSE,
                                                 TRUE))
	...
	# define true positives
	biggest_4_subsets <- sort(unique(fdn_found_in_selected_subsamples$target_MEND_count),
	                          decreasing = TRUE)[1:4]
  ...
  true_neighbors_in_parent_sample <- neighbor_frequency %>% 
    ...
    pull(neighbor_sample)
      ...
      true_neighbors_in_parent_sample <- subset(first_degree_results_filtered,
  		 parent_sample == this_parent_sample & 
  		 target_MEND_count == 50) %>%
      dplyr::filter(sample_is_neighbor_in_subset) %>% 
      pull(neighbor_sample)
	  }
  
	 if(this_truth_definition == "found_in_at_least_half_the_subsets") {
	   minimum_hits_for_true_neighbors <- length(unique(
	     fdn_found_in_selected_subsamples$focus_sample)) / 2
	   neighbor_frequency <- fdn_found_in_selected_subsamples %>%
	     dplyr::filter(sample_is_neighbor_in_subset) %>% 
	     group_by(neighbor_sample) %>% 
  ...
  if(this_truth_definition != "found_in_the_deepest_subset") {
	...
	this_annotated_fdn_results <- fdn_results %>% 
		...
		mutate(
			...
			sample_is_true_positive_neighbor_in_parent_sample = neighbor_sample %in%
...
#		          by = c("focus_sample"="sample_id")
#		) %>%
  left_join(first_degree_neighbor_result_codes,
            by = c("sample_is_neighbor_in_subset",
...
ordered_plot_input <- annotated_fdn_results %>% 
  ...
  mutate(parent_sample_label = paste0(
    parent_sample, " (n=", length(unique(unique_focus_neighbor_id)), ")"))
...
##  What percent of correlated samples were detectable with 1 million MEND reads?

```{r}

# how many (real) correlated samples are there? 59
subset(ordered_plot_input, median_of_depth_bin == 0) %>%
  pull(unique_focus_neighbor_id) %>% unique %>% length

# 2% of (real) correlated samples were detectable with 1 million MEND reads
subset(ordered_plot_input, median_of_depth_bin == 0 &
         sample_is_true_positive_neighbor_in_parent_sample) %>%
  tabyl(first_appears)
...
g_frac_accuracy <- gather(
  frac_accuracy, fraction_type, fraction_amount, -median_of_depth_bin) %>%
...
p4 <- ggplot(g_frac_accuracy) + 
  geom_bar(aes(x = median_of_depth_bin,
               y = fraction_amount / number_of_parent_samples,
               fill = hit_type),
           stat = "identity") +  
	scale_fill_manual(values = plot_colors) +
  scale_x_continuous("MEND reads (million)",
                     breaks = outlier_results_x_breaks,
                     labels = outlier_results_x_labels) +
...
#"tp_count_in_subset", "count_of_positives_in_full_set", "fp_count_in_subset", "count_of_negatives_in_full_set", "all_positives_in_subset", "allNegatives", 
...
for (this_parent_sample in unique(ordered_plot_input$parent_sample)) {
  ...
  count_of_positives_in_full_set <- length(unique(subset(
    this_plot_input,
    sample_is_true_positive_neighbor_in_parent_sample)$neighbor_sample))
  ...
  g_accuracy_stats_this_parent <- gather(
    accuracy_stats_this_parent[,colsToGather], measurement, value, -median_of_depth_bin) %>%
...
accuracy_stats_min <- subset(accuracy_stats, measurement %in% c("sensitivity", "specificity") ) %>%
  ...
  mutate(depth_bin_min_value = min(value),
         point_label = ifelse(depth_bin_min_value == value,
                              paste0("x=", median_of_depth_bin, " y=", round(value, 3)),"")) 

proposed_thresholds_raw <- tibble(threshold = c(0.995, 0.95),
                                  measurement = c("specificity", "sensitivity"),
                                  min_reads = 20 )

proposed_thresholds <- lapply(unique(accuracy_stats$parent_sample), function(x) {
  proposed_thresholds_raw %>%
    mutate(parent_sample = x)
}) %>% bind_rows

if(f_make_supporting_plots) {
  ggplot(accuracy_stats_min, aes(y = value, color = measurement, x = median_of_depth_bin)) +
    geom_point() +
    geom_smooth(se = FALSE) +
    ggtitle(paste("sensitivity and specificity per parent sample")) +
    facet_wrap(parent_sample~measurement, scale = "free_y", ncol = 2) +
...
samples_tested_for_correlation_in_assay <- disease_info_for_compendium %>%
  dplyr::filter(disease %in% ordered_plot_input$diagnosis) %>%
  nrow


count_of_truly_NON_correlated_samples <-
  samples_tested_for_correlation_in_assay - count_of_truly_correlated_samples
...
samples_NEVER_called_as_neighbor <-
  samples_tested_for_correlation_in_assay - samples_ever_called_as_neighbor
...
accuracy_stats_raw <- accuracy_counts %>%
  mutate(sensitivity = tp_count / count_of_truly_correlated_samples,
...
pCSS <- ggplot(accuracy_stats_combined_with_no_zeroes,
               aes(y = value, color = measurement, x = median_of_depth_bin)) + 
  geom_point() +
  geom_smooth(se = TRUE) + 
  scale_color_manual(values = accuracy_line_colors) + 
  theme(legend.position = "none") +
  xlab("MEND reads (million)" ) +
  ylim(0, 1.1) +
  scale_y_continuous("Fraction", breaks = seq(0, 1, 0.2))
...
ggsave(file = paste0("../figures_and_tables/",
                     ...
                     "_",
                     gsub(" ", "_", gsub(":", "-", Sys.Date())),
                     "_",
                     "UTC",
                     ".png"),
       grobCombo,  width = 10, height = 8)
