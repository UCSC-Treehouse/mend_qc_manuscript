...
literature_search_inputs <- parent_outliers %>% 
  ...
  mutate(query_text = paste0(Disease," AND ", gene, "[Gene]"))
...
# literature_search_inputs

pub_tib <- lapply(literature_search_inputs$query_text, function(query_t){
  ...
  pmids <- esearch(query_t, "pubmed", usehistory = TRUE)
  if ( ! pmids$xmlValue("/eSearchResult/Count")==0) {
    es <- esummary(pmids,  db="pubmed")
    parseeee <- content(es, "parsed")
    pub_list <- lapply(names(parseeee), function(pub_id){
      # one_cite = parseeee [[1]]
      one_cite <- parseeee[[pub_id]]
      names_keys = grepl("Authors.Author.Name", names(one_cite))
      concat_names = one_cite[names_keys] %>% unlist %>% paste(collapse=", ")
      # one_cite[["Authors.Author.Name"]]
      #    one_cite[c("FullJournalName", "Title")] %>% unlist %>% as_tibble
      tibble(journal = unlist(one_cite[["FullJournalName"]]),
             article_title = unlist(one_cite[["Title"]]),
             authors=concat_names,
             publication_date = unlist(one_cite[["PubDate"]]),
             reference_count = unlist(one_cite[["PmcRefCount"]]),
             PMID = pub_id
      )
    }) %>% bind_rows
  }
...
# })
# 
# query <- "embryonal rhabdomyosarcoma BOD1"
# query <- "BOD1"
# pmids <- esearch(query, "pubmed", usehistory = TRUE)
# 
# pmids$xmlValue("/eSearchResult/Count")
# 
# if (pmids$xmlValue("/eSearchResult/Count"))
# 
# articles <- efetch(pmids)
# 
# # example hit for BOD1
# # Nat Rev Mol Cell Biol. 2005 Dec;6(12):929-42.
# # Kinetochore capture and bi-orientation on the mitotic spindle.
# # Tanaka TU1, Stark MJ, Tanaka K.
# # PMID 16341079
# es <- esummary("16341079",  db="pubmed")
# 
# es$xmlValue("//Source")
# es$xmlValue("//PmcRefCount")
# es$xmlValue("//Title")
...
pubmed_results_list <- lapply(literature_search_inputs$query_text, function(query_t){
  ...
  if (this_search$count>0){
    ...
    t5 <- extract_from_esummary(multi_summs, c("pmcrefcount", "title", "source", "pubdate" )) %>%
      t %>% 
      apply(1, as_tibble) 
...
pubmed_results_no_controls <- subset(pubmed_results, ! query_text == "synovial sarcoma brca1") %>%
#  select(-PMID, query_text, title,  source, PMid, pubdate, pmcrefcount) %>% 
  arrange(query_text, desc(pmcrefcount))
  
write_tsv(pubmed_results_no_controls, "../ancillary/all_pubmed_search_results_for_outliers_by_disease.txt")  
write_tsv(subset(pubmed_results_no_controls,! is.na(pubdate)), "../ancillary/positive_pubmed_search_results_for_outliers_by_disease.txt")  
#        
#        (pubmed_results)
