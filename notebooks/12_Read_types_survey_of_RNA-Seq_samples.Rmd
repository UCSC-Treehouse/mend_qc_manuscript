...
## Define reference ranges based on mean and sd of samples with typical total read depths and at least 20M umend reads 
...
We addressed variability in sample composition with reference ranges for four types of reads found in sequencing data: unmapped (0-27%); mapped duplicate (0-57%); mapped non exonic (0-27%) and uniquely mapped, exonic, non-duplicate (UMEND, 21-77%). 
...
t5 <- adjusted_UMEND_wnl %>% filter(total_adjusted_read_count < example_threshold) %>% nrow
t5/nrow(adjusted_UMEND_wnl)

print(paste("All samples in this figure and", round(100*t5/nrow(adjusted_UMEND_wnl), 1), "% of the", number_of_samples_within_reference_ranges, "samples in the reference range require fewer than", round(example_threshold/1E6), "M total reads to contain 20M UMEND reads (dashed horizontal line)."))
...
plot_info_for_problem_samples %>%
  rename(UMEND_depth = status) %>%
  tabyl(read_type, within_limits, UMEND_depth) 
  
  
# summary statements
samples_with_lt_10M_UMEND_reads <- read_counts_with_limits %>%
  filter(read_type == "UMEND",
         read_count<10E6) %>%
  pull (sample_id)

print(paste(length(samples_with_lt_10M_UMEND_reads), "samples have fewer than 10M UMEND reads"))

samples_with_10_20_M_UMEND_reads <- read_counts_with_limits %>%
  filter(read_type == "UMEND",
         read_count >= 10E6,
         read_count < 20E6) %>%
  pull (sample_id)

print(paste(length(samples_with_10_20_M_UMEND_reads), "samples have 10-20 M UMEND reads"))

samples_with_gt_20_M_UMEND_reads <- read_counts_with_limits %>%
  filter(read_type == "UMEND",
         read_count >= 20E6) %>%
  pull (sample_id)

print(paste(length(samples_with_gt_20_M_UMEND_reads), "samples have >20 M UMEND reads"))


samples_with_gt_20_M_UMEND_reads_not_WNL <- read_counts_with_limits %>%
  filter(read_type == "UMEND",
         read_count >= 20E6,
         ! all_read_types_within_read_type_limits) %>%
  pull (sample_id)

print(paste(length(samples_with_gt_20_M_UMEND_reads_not_WNL), "of the", length(samples_with_gt_20_M_UMEND_reads), "samples with >20 M UMEND reads are not within reference range for all five read types"))

print(paste(round(100*length(samples_with_gt_20_M_UMEND_reads_not_WNL)/length(samples_with_gt_20_M_UMEND_reads), 1), "percent"))
...
plot_info_for_problem_samples   %>%
  ...
  nrow
# 
# how many samples above 20M UMEND reads, would otherwise pass, but have excessive multi-mappers?
# 30
plot_info_for_problem_samples   %>%
