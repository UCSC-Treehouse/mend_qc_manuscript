...
literature_outliers <- read_tsv("../ancillary/genes_with_gt_10_journal_articles.tsv") %>%
...
annotated_outlier_results <- lapply(unique(outlier_results_raw$parent_sample)[2:5], function(this_parent_sample) {
  ...
  if (this_truth_definition != "found_in_the_deepest_subset") {
	...
	true_outliers_in_parent_sample <- literature_outliers %>% 
	  filter(Eval_ID == this_parent_sample) %>%
	  pull(gene)
...
table(ordered_plot_input$gene_is_true_positive_in_parent_sample)
...
```{r}
lit_plot_input <- subset(ordered_plot_input, gene_is_true_positive_in_parent_sample)


plot_title="outliers - TH_EVal_019 - each row is a gene"  
pIA<-
  
  ggplot(subset(lit_plot_input), aes(x = median_of_depth_bin, y = ordered_gene, fill = hit_type)) + 
